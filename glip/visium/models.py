...
def _resolve_image_embedding(image_encoder, image_embedding):
    if image_embedding is not None:
        return int(image_embedding)

    output_dim = getattr(image_encoder, "output_dim", None)
    if output_dim is None:
        raise AttributeError("Image encoder does not expose an output_dim for the projection head.")
...
class CLIPModel_UNI(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.image_encoder = ImageEncoder_UNI(
            pretrained=pretrained,
            checkpoint_path=checkpoint_path,
        )
        self.image_projection = ProjectionHead(

#!/usr/bin/env python3
"""Run 5 Xenium single-cell folds into a single parent directory."""
...
TRAIN_SCRIPT = "/data/yujk/GLIP/train_xenium.py"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run all 5 Xenium single-cell folds into one parent directory")
    ...
    parser.add_argument("--final-train-eval-max-cells", type=int, default=4096)
    parser.add_argument("--final-test-eval-max-cells", type=int, default=0)
    ...
    for fold_index in range(args.num_position_folds):
        ...
        print(f"\n=== Running Xenium single-cell fold {fold_index} ===", flush=True)
        cmd = [
            sys.executable,
            TRAIN_SCRIPT,
            "--run-dir",
            ...
            args.device,
            "--max-train-cells",
            str(args.max_train_cells),
            "--max-test-cells",
            str(args.max_test_cells),
            "--final-train-eval-max-cells",
            str(args.final_train_eval_max_cells),
            "--final-test-eval-max-cells",
            str(args.final_test_eval_max_cells),
        ]
